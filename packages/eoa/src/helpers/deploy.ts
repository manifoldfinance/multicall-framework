...
export async function isCanonicalDeployed(provider: providers.Provider) {
  try {
    await checkCode(
      provider,
      safe120Config.safeAddress,
      safe120Config.runtimeCode
    );
  } catch (e) {
...
export async function deployCanonicals(signer: Signer) {
  ...
  const deploy111AndFactory = async () => {
    ...
    const deploymentCosts111AndFactory = BigNumber.from(
      safe111AndFactoryConfig.deploymentCosts
    );
  ...
  const deploy120 = async () => {
    ...
    log(
      "price: ",
      utils.formatEther(deploymentCosts120.sub(deploymentAccountBalance))
    );
  ...
  const deployTx = async (txConfig: SavedTx) => {
    ...
    const deploymentCostsDefaultHandler = BigNumber.from(
      txConfig.deploymentCosts
    );
    const deploymentAccountBalance = await provider.getBalance(
      txConfig.deployer
    );
    if (deploymentAccountBalance.lt(deploymentCostsDefaultHandler)) {
      log(
        "sending ",
        deploymentCostsDefaultHandler.sub(deploymentAccountBalance).toString(),
        " to deployer"
      );
      const tx = await funder.sendTransaction({
  ...
  return deploy111AndFactory()
    .then(deploy120)
    .then(() => {
      return chain([tx2, tx3, tx4, tx5, tx6, tx7, tx8, tx9, tx10]);
    });
}
