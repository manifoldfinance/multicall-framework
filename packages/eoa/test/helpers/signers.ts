...
export const signerFor = async (
  ...
  signerOrAddress: SignerWithAddress | string
): Promise<SignerWithAddress> => {
  if (typeof (signerOrAddress as any).sendTransaction == "function")
    return signerOrAddress as SignerWithAddress;

  const signer = (await hre.ethers.getSigners()).find(
    (signer) => signer.address === signerOrAddress
  );
